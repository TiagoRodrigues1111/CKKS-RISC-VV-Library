        ...
        for(uint8_t i=0;i<rns_number;i++)
        {
                for(uint32_t j=0;j<stop_value;j++)
                {
                        ...
                        while(plaintext_value_reduced_aux < 0)
                                plaintext_value_reduced_aux += Ciphertext_result->coefficient_modulus[i];
        
                        // If plaintext value bigger than the ring, reduce it into the ring
                        while(plaintext_value_reduced_aux >= Ciphertext_result->coefficient_modulus[i])
                                plaintext_value_reduced_aux -= Ciphertext_result->coefficient_modulus[i];
                        
                        // Add Ciphertext and plaintext polynomials, and reduce to ring 
                        Ciphertext_result->values[0][i][j] = Ciphertext1.values[0][i][j] + plaintext_value_reduced_aux;
                        ...
                        // Copy Ciphertext1 second polynomial to Ciphertext_result
                        Ciphertext_result->values[1][i][j] = Ciphertext1.values[1][i][j]; 
               }
        }
        return 0;
}
...
int CADD_mod_comp(struct ciphertext Ciphertext1,struct plaintext plaintext1, struct ciphertext *Ciphertext_result)
{
        ...
        for(uint8_t i=0;i<rns_number;i++)
        {
                ...
                for(uint32_t j=0;j<stop_value;j++)
                {
                        ...
                        if(plaintext1.values[j]<0)
                        {
                                plaintext1.values[j] = -plaintext1.values[j];
                                plaintext1.values[j] = plaintext1.values[j] % Ciphertext_result->coefficient_modulus[i];
                                if(plaintext1.values[j] != 0)
                                    plaintext1.values[j] -= Ciphertext_result->coefficient_modulus[i];   
        ...
        {       
                for(uint32_t j=0;j<stop_value;j++)
                {
                        ...
                        if(plaintext_value_reduced_aux<0)
                        {
                                ...
                                uint64_t auxq = ((plaintext_value_reduced_aux * Ciphertext_result->barrett_auxi_value[i]) >> 45);
                                plaintext_value_reduced_aux -= auxq * Ciphertext_result->coefficient_modulus[i];
                                if (Ciphertext_result->coefficient_modulus[i] <= plaintext_value_reduced_aux) 
                                        plaintext_value_reduced_aux -= Ciphertext_result->coefficient_modulus[i];
...
int CADD_naive_vect(struct ciphertext Ciphertext1,struct plaintext plaintext1, struct ciphertext *Ciphertext_result)
{
        ...
        for(uint8_t i=0;i<rns_number;i++)
        {
                ...
                for(uint32_t j=0;j<stop_value;j+=gvl)
                {
                      ...
                      //  __epi_1xi1 mask_pt_reduce =__builtin_epi_vmsgt_1xi64(mask_comp_neg,v_plaintext , gvl);
                        ...
                        __epi_1xi64 v_res_1 = __builtin_epi_vadd_1xi64(v_ciphertext1_0, v_plaintext, gvl);
                        ...
                        __builtin_epi_vstore_unsigned_1xi64(&Ciphertext_result->values[1][i][j], v_ciphertext1_1, gvl);
                }
        }
        return 0;
}
...
int CADD_naive_vect_test1(struct ciphertext Ciphertext1,struct plaintext plaintext1, struct ciphertext *Ciphertext_result)
{
        ...
        for(uint8_t i=0;i<rns_number;i++)
        {
                ...
                for(uint32_t j=0;j<stop_value;j+=gvl)
                {
                        ...
                        {                       
                                v_plaintext = __builtin_epi_vadd_1xi64_mask(v_plaintext, v_plaintext,v_coef_mod, mask_pt_reduce, gvl);
                                mask_pt_reduce =__builtin_epi_vmsgt_1xi64(mask_comp_neg,v_plaintext, gvl);
                        }
...
int CADD_naive_vect_test2(struct ciphertext Ciphertext1,struct plaintext plaintext1, struct ciphertext *Ciphertext_result)
{
        ...
        for(uint8_t i=0;i<rns_number;i++)
        {
                ...
                for(uint32_t j=0;j<stop_value;j+=gvl)
                {
                        ...
                        // Reduction plaintext part-2
                       __epi_1xi1  mask_pt_reduce = __builtin_epi_vmsleu_1xi64(v_coef_mod,v_plaintext, gvl);   
                        while((__builtin_epi_vmpopc_1xi1(mask_pt_reduce,gvl))!=0)
                        {                       
                                v_plaintext = __builtin_epi_vsub_1xi64_mask(v_plaintext, v_plaintext,v_coef_mod, mask_pt_reduce, gvl);
                                mask_pt_reduce = __builtin_epi_vmsleu_1xi64(v_coef_mod,v_plaintext, gvl);
...
int CADD_naive_vect_unroll(struct ciphertext Ciphertext1,struct plaintext plaintext1, struct ciphertext *Ciphertext_result)
{
        ...
        for(uint8_t i=0;i<rns_number;i++)
        {
                ...
                for (j=0;j<loops;j++) 
                {
                        ...
                        __epi_1xi64 v_res_1 = __builtin_epi_vadd_1xi64(v_ciphertext1_0_0, v_plaintext_0_0, max_gvl);
                        __epi_1xi64 v_res_2 = __builtin_epi_vadd_1xi64(v_ciphertext1_0_1, v_plaintext_0_1, max_gvl);
                        ...
                        __builtin_epi_vstore_unsigned_1xi64(&Ciphertext_result->values[1][i][(aux1+0)*max_gvl], v_ciphertext1_1_0, max_gvl);
                        __builtin_epi_vstore_unsigned_1xi64(&Ciphertext_result->values[1][i][(aux1+1)*max_gvl], v_ciphertext1_1_1, max_gvl);       
                ...
                for (j = start_remain; j < Ciphertext_result->polynomial_degree_modulus; j += gvl) 
                {               
                        gvl = __builtin_epi_vsetvl(Ciphertext_result->polynomial_degree_modulus - j, __epi_e64, __epi_m1);
...
{        
        ...
        uint32_t gvl = __builtin_epi_vsetvl(Ciphertext_result->polynomial_degree_modulus, __epi_e64, __epi_m1);  
        for(uint8_t i=0; i<rns_number;i++)
        {
...
{        
        ...
        for(uint8_t i=0; i<rns_number;i++)
        {
                ...
                for (uint32_t j=0; j<stop_value;j+=gvl) 
                {
                        ...
                        // Reduction plaintext
                        __epi_1xi64 v_q = __builtin_epi_vmulhu_1xi64(v_aux_neg, v_m, gvl);
                        v_q =  __builtin_epi_vsrl_1xi64(v_q, v_shift_val , gvl);
                        __epi_1xi64 v_aux1 = __builtin_epi_vmul_1xi64(v_q, v_coef_mod, gvl);
                        v_aux_neg = __builtin_epi_vsub_1xi64(v_aux_neg, v_aux1, gvl);
                        __epi_1xi1 mask = __builtin_epi_vmsleu_1xi64(v_coef_mod, v_aux_neg, gvl);
...
{        
        ...
        for(uint8_t i=0; i<rns_number;i++)
        {
                ...
                for (uint32_t j=0; j<stop_value;j+=gvl) 
                {
                        ...
                        // Reduction plaintext
                        __epi_1xi64 v_q = __builtin_epi_vmul_1xi64(v_plaintext, v_m, gvl);
                        v_q =  __builtin_epi_vsrl_1xi64(v_q, v_shift_val , gvl);
                        __epi_1xi64 v_aux1 = __builtin_epi_vmul_1xi64(v_q, v_coef_mod, gvl);
                        v_plaintext = __builtin_epi_vsub_1xi64(v_plaintext, v_aux1, gvl);
                        __epi_1xi1 mask = __builtin_epi_vmsleu_1xi64(v_coef_mod, v_plaintext, gvl);
...
{       
        ...
        for(uint8_t i=0;i<rns_number;i++ )
        {
                ...
                {     
                        ...
                        // Load plaintext
                        __epi_1xi64 v_plaintext = __builtin_epi_vload_unsigned_1xi64((uint64_t*)&plaintext1.values[j], gvl);
