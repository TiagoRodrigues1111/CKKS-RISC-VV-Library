        ...
        for(uint8_t i=0;i<rns_number;i++ )
        {
                ...
                for(uint32_t j=0;j<stop_value;j++)
                {
                        ...
                        while(plaintext_values_reduced_aux[j] < 0)
                                plaintext_values_reduced_aux[j] += Ciphertext1->coefficient_modulus[i];
        
                        // If plaintext value bigger than the ring, reduce it into the ring
                        while(plaintext_values_reduced_aux[j] >= Ciphertext1->coefficient_modulus[i])
                                plaintext_values_reduced_aux[j] -= Ciphertext1->coefficient_modulus[i];
        ...
        for(uint8_t i=0;i<rns_number;i++ )
        {
                ...
                for (uint32_t j=0;j<stop_value;j++)
                {
                        ...
                        if(plaintext_values_reduced_aux[j]<0)
                        {
                                ...
                                uint64_t auxq = ((plaintext_values_reduced_aux[j] * Ciphertext1->barrett_auxi_value[i]) >> 45);
                                plaintext_values_reduced_aux[j] -= auxq * Ciphertext1->coefficient_modulus[i];
                                if (Ciphertext1->coefficient_modulus[i] <= plaintext_values_reduced_aux[j]) 
                                        plaintext_values_reduced_aux[j] -= Ciphertext1->coefficient_modulus[i];
...
int CMULT_barrett_vect_unroll_inplace(struct ciphertext *Ciphertext1,struct plaintext plaintext1)
{
        ...
        for(uint8_t i=0;i<rns_number;i++)
        {
                ...
                for (j = 0; j<loops;j++) 
                {
                        ...
                        __epi_1xi1 v_mask = __builtin_epi_vmslt_1xi64(v_plaintext_0, mask_comp_neg, gvl);
                        __epi_1xi64 v_aux_neg = __builtin_epi_vsub_1xi64_mask(v_plaintext_0,mask_comp_neg,v_plaintext_0, v_mask, gvl);
                        ...
                        v_aux_neg = __builtin_epi_vsub_1xi64_mask(v_aux_neg, v_aux_neg,v_coef_mod, last_mask, gvl);     
                        v_plaintext_0 = __builtin_epi_vsub_1xi64_mask(v_aux_neg,mask_comp_neg,v_aux_neg,last_mask, gvl); 


                        // Make mask for negative values in plaintext 
                        
                         v_mask = __builtin_epi_vmslt_1xi64(v_plaintext_1, mask_comp_neg, gvl);
                         v_aux_neg = __builtin_epi_vsub_1xi64_mask(v_plaintext_1,mask_comp_neg,v_plaintext_1, v_mask, gvl);
                                                             
                        // Reduction plaintext
                        v_q = __builtin_epi_vmul_1xi64(v_aux_neg, v_m, gvl);
                        v_q =  __builtin_epi_vsrl_1xi64(v_q, v_shift_val , gvl);
                        v_aux1 = __builtin_epi_vmul_1xi64(v_q, v_coef_mod, gvl);
                        v_aux_neg = __builtin_epi_vsub_1xi64(v_aux_neg, v_aux1, gvl);
                        mask = __builtin_epi_vmsleu_1xi64(v_coef_mod, v_aux_neg, gvl);
                        v_aux_neg = __builtin_epi_vsub_1xi64_mask(v_aux_neg, v_aux_neg, v_coef_mod, mask, gvl);
                        
                        zero_mask = __builtin_epi_vmseq_1xi64(v_aux_neg, mask_comp_neg, gvl);
                        last_mask = __builtin_epi_vmandnot_1xi1(v_mask, zero_mask, gvl);
                               
                        v_aux_neg = __builtin_epi_vsub_1xi64_mask(v_aux_neg, v_aux_neg,v_coef_mod, last_mask, gvl);     
                        v_plaintext_1 = __builtin_epi_vsub_1xi64_mask(v_aux_neg,mask_comp_neg,v_aux_neg,last_mask, gvl);  
