#include "HMULT_inplace.h"

int HMULT_naive_inplace(struct ciphertext *Ciphertext1,struct ciphertext Ciphertext2)
{
        ...
        if(Ciphertext1->number_of_polynomials==2)
        {
                uint64_t ***realloc_auxiliary = NULL;     
                realloc_auxiliary =  (uint64_t***) realloc(Ciphertext1->values,(Ciphertext1->number_of_polynomials+1)*sizeof(uint64_t**));
                if (realloc_auxiliary == NULL)        
                        return 3;
    
                Ciphertext1->values = realloc_auxiliary;      
        
                Ciphertext1->values[Ciphertext1->number_of_polynomials] = (uint64_t**) malloc(Ciphertext1->rns_number*sizeof(uint64_t*));    // if using += operations, change to calloc
                
                if(Ciphertext1->values[Ciphertext1->number_of_polynomials] == NULL)   
                        return 4;        
                for(uint8_t i=0;i<rns_number;i++)
                {
                         Ciphertext1->values[Ciphertext1->number_of_polynomials][i] = (uint64_t*) malloc(Ciphertext1->polynomial_degree_modulus*sizeof(uint64_t));
                         if(Ciphertext1->values[Ciphertext1->number_of_polynomials][i] == NULL)
                         {
                               return 5;   
                         }
                }   
                Ciphertext1->number_of_polynomials++;
        }
        ...
        for(uint8_t i=0;i<rns_number;i++ )
        {
                ...
                for (uint32_t j=0;j<stop_value;j++)
                {
                        ...
                        while(auxiliary_array[0][j] >= Ciphertext1->coefficient_modulus[i])
                                auxiliary_array[0][j] -= Ciphertext1->coefficient_modulus[i];   
                                                 
                        while(auxiliary_array[0][stop_value+j] >= Ciphertext1->coefficient_modulus[i])
                                auxiliary_array[0][stop_value+j] -= Ciphertext1->coefficient_modulus[i];        
                             
                             
                        while(auxiliary_array[1][j] >= Ciphertext1->coefficient_modulus[i])
                                auxiliary_array[1][j] -= Ciphertext1->coefficient_modulus[i];   
                                                 
                        while(auxiliary_array[1][stop_value+j] >= Ciphertext1->coefficient_modulus[i])
                                auxiliary_array[1][stop_value+j] -= Ciphertext1->coefficient_modulus[i];  


                        Ciphertext1->values[0][i][j] = auxiliary_array[0][j] - auxiliary_array[0][stop_value+j];
                        if(Ciphertext1->values[0][i][j] >= Ciphertext1->coefficient_modulus[i])
                                Ciphertext1->values[0][i][j] += Ciphertext1->coefficient_modulus[i];
                        
                        Ciphertext1->values[1][i][j] = auxiliary_array[1][j] - auxiliary_array[1][stop_value+j];
...
int HMULT_naive_auto_vect_inplace(struct ciphertext *Ciphertext1,struct ciphertext Ciphertext2)
{
        const uint32_t stop_value = Ciphertext1->polynomial_degree_modulus;
        const uint8_t rns_number = Ciphertext1->rns_number; 
...
int HMULT_barrett_inplace(struct ciphertext *Ciphertext1,struct ciphertext Ciphertext2)
{
        const uint32_t stop_value = Ciphertext1->polynomial_degree_modulus;
        const uint8_t rns_number = Ciphertext1->rns_number; 
        if(Ciphertext1->polynomial_degree_modulus <= 0)
        ...
        for(uint8_t i=0;i<rns_number;i++ )
        {
                ...
                for (uint32_t j=0;j<stop_value;j++)
                {
                        uint64_t auxq = (auxiliary_array[0][j] * Ciphertext1->barrett_auxi_value[i]) >> 45;
                        auxiliary_array[0][j] -= auxq * Ciphertext1->coefficient_modulus[i];
                        if(Ciphertext1->coefficient_modulus[i] <= auxiliary_array[0][j]) 
                               auxiliary_array[0][j] -= Ciphertext1->coefficient_modulus[i];                       

                        auxq = (auxiliary_array[0][stop_value+j] * Ciphertext1->barrett_auxi_value[i]) >> 45;
                        auxiliary_array[0][stop_value+j] -= auxq * Ciphertext1->coefficient_modulus[i];
                        if(Ciphertext1->coefficient_modulus[i] <= auxiliary_array[0][stop_value+j]) 
                               auxiliary_array[0][stop_value+j] -= Ciphertext1->coefficient_modulus[i];    


                        auxq = (auxiliary_array[1][j] * Ciphertext1->barrett_auxi_value[i]) >> 45;
                        auxiliary_array[1][j] -= auxq * Ciphertext1->coefficient_modulus[i];
                        if(Ciphertext1->coefficient_modulus[i] <= auxiliary_array[1][j]) 
                               auxiliary_array[1][j] -= Ciphertext1->coefficient_modulus[i];                       

                        auxq = (auxiliary_array[1][stop_value+j] * Ciphertext1->barrett_auxi_value[i]) >> 45;
                        auxiliary_array[1][stop_value+j] -= auxq * Ciphertext1->coefficient_modulus[i];
                        if(Ciphertext1->coefficient_modulus[i] <= auxiliary_array[1][stop_value+j]) 
                               auxiliary_array[1][stop_value+j] -= Ciphertext1->coefficient_modulus[i]; 


                        auxq = (auxiliary_array[2][j] * Ciphertext1->barrett_auxi_value[i]) >> 45;
                        auxiliary_array[2][j] -= auxq * Ciphertext1->coefficient_modulus[i];
                        if(Ciphertext1->coefficient_modulus[i] <= auxiliary_array[2][j]) 
                               auxiliary_array[2][j] -= Ciphertext1->coefficient_modulus[i];                       

                        auxq = (auxiliary_array[2][stop_value+j] * Ciphertext1->barrett_auxi_value[i]) >> 45;
                        auxiliary_array[2][stop_value+j] -= auxq * Ciphertext1->coefficient_modulus[i];
                        if(Ciphertext1->coefficient_modulus[i] <= auxiliary_array[2][stop_value+j]) 
                               auxiliary_array[2][stop_value+j] -= Ciphertext1->coefficient_modulus[i];
                        ...
                        Ciphertext1->values[2][i][j] = auxiliary_array[2][j] - auxiliary_array[2][stop_value+j];
                        if(Ciphertext1->values[2][i][j] >= Ciphertext1->coefficient_modulus[i])
                                Ciphertext1->values[2][i][j] += Ciphertext1->coefficient_modulus[i];
                }                   
        }
        return 0;       
}

int HMULT_barrett_auto_vect_inplace(struct ciphertext *Ciphertext1,struct ciphertext Ciphertext2)
{
        const uint32_t stop_value = Ciphertext1->polynomial_degree_modulus;
        const uint8_t rns_number = Ciphertext1->rns_number; 
        if(Ciphertext1->polynomial_degree_modulus <= 0)
...
int HMULT_naive_vect_inplace(struct ciphertext *Ciphertext1,struct ciphertext Ciphertext2)
{
        const uint32_t stop_value = Ciphertext1->polynomial_degree_modulus;
        const uint8_t rns_number = Ciphertext1->rns_number; 
        ...
        for(uint8_t i=0; i<rns_number;i++)
        {
                ...
                for (uint32_t j = 0; j < stop_value; j += gvl) 
                {
                        ...
                        __builtin_epi_vstore_unsigned_1xi64(&Ciphertext1->values[2][i][j], v_res_3, gvl);
...
int HMULT_naive_vect_unroll_inplace(struct ciphertext *Ciphertext1,struct ciphertext Ciphertext2)
{
        const uint32_t stop_value = Ciphertext1->polynomial_degree_modulus;
        const uint8_t rns_number = Ciphertext1->rns_number;   
...
int HMULT_barrett_vect_inplace(struct ciphertext *Ciphertext1,struct ciphertext Ciphertext2)
{
        const uint32_t stop_value = Ciphertext1->polynomial_degree_modulus;
        const uint8_t rns_number = Ciphertext1->rns_number; 
        ...
        for(uint8_t i=0;i<rns_number;i++)
        {
                ...
                {  
                        ...
                        // Reduction aux_result2_0
                        v_q = __builtin_epi_vmulhu_1xi64(v_aux_result_2_0, v_m, gvl);
                        v_q =  __builtin_epi_vsrl_1xi64(v_q, v_shift_val , gvl);
                        v_aux1 = __builtin_epi_vmul_1xi64(v_q, v_coef_mod, gvl);
                        ...
                        v_aux_result_2_0 = __builtin_epi_vsub_1xi64_mask(v_aux_result_2_0, v_aux_result_2_0, v_coef_mod, mask, gvl);
                         ...
                         // Reduction aux_result2_1
                        v_q = __builtin_epi_vmulhu_1xi64(v_aux_result_2_1, v_m, gvl);
                        v_q =  __builtin_epi_vsrl_1xi64(v_q, v_shift_val , gvl);
                        v_aux1 = __builtin_epi_vmul_1xi64(v_q, v_coef_mod, gvl);
...
int HMULT_barrett_vect_unroll_inplace(struct ciphertext *Ciphertext1,struct ciphertext Ciphertext2)	
{
        const uint32_t stop_value = Ciphertext1->polynomial_degree_modulus;
        const uint8_t rns_number = Ciphertext1->rns_number; 
        const uint8_t unroll = 2;      
